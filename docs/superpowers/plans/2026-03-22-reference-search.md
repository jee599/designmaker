# reference-search 스킬 구현 계획

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `/reference-search` 슬래시 커맨드를 만들어 디자인 갤러리에서 레퍼런스를 자동 탐색+큐레이션+등록하는 파이프라인을 구현한다.

**Architecture:** Claude Code 슬래시 커맨드(.claude/commands/reference-search.md)가 핵심. JSON 데이터 파일(sources.json, seen.json)로 상태 관리, taste-profile.md로 취향 누적. 스킬 실행 시 WebSearch/WebFetch로 갤러리 탐색 → HTML 분석 → 품질 게이트 → 인터랙티브 큐레이션 → 카탈로그 등록 → 자기 회고.

**Tech Stack:** Claude Code slash command (Markdown prompt), JSON, WebSearch, WebFetch

**Spec:** `docs/superpowers/specs/2026-03-22-reference-search-design.md`

---

### Task 1: 데이터 파일 생성 (sources.json, seen.json, taste-profile.md)

**Files:**
- Create: `web/data/sources.json`
- Create: `web/data/seen.json`
- Create: `web/data/taste-profile.md`

- [ ] **Step 1: Create sources.json with 20 gallery sources**

```json
[
  { "name": "Awwwards", "searchQuery": "site:awwwards.com site of the day {year}-{month}", "enabled": true },
  { "name": "CSSDA", "searchQuery": "site:cssdesignawards.com website of the day {year} {month}", "enabled": true },
  { "name": "SiteInspire", "searchQuery": "site:siteinspire.com {year} {month}", "enabled": true },
  { "name": "Godly", "searchQuery": "site:godly.website {year}", "enabled": true },
  { "name": "Mindsparkle", "searchQuery": "site:mindsparklemag.com {year} {month} web design", "enabled": true },
  { "name": "The FWA", "searchQuery": "site:thefwa.com site of the day {year}", "enabled": true },
  { "name": "Httpster", "searchQuery": "site:httpster.net {year}", "enabled": true },
  { "name": "Lapa Ninja", "searchQuery": "site:lapa.ninja {year} landing page", "enabled": true },
  { "name": "Land-book", "searchQuery": "site:land-book.com {year}", "enabled": true },
  { "name": "One Page Love", "searchQuery": "site:onepagelove.com {year} {month}", "enabled": true },
  { "name": "Best Website Gallery", "searchQuery": "site:bestwebsite.gallery {year}", "enabled": true },
  { "name": "Muzli", "searchQuery": "site:search.muz.li web design inspiration {year}", "enabled": true },
  { "name": "Webdesign Inspiration", "searchQuery": "site:webdesign-inspiration.com {year}", "enabled": true },
  { "name": "Minimal Gallery", "searchQuery": "site:minimal.gallery {year}", "enabled": true },
  { "name": "Dark Mode Design", "searchQuery": "site:darkmodedesign.com {year}", "enabled": true },
  { "name": "Brutalist Websites", "searchQuery": "site:brutalistwebsites.com {year}", "enabled": true },
  { "name": "Mobbin", "searchQuery": "site:mobbin.com web {year}", "enabled": true },
  { "name": "Screenlane", "searchQuery": "site:screenlane.com {year}", "enabled": true },
  { "name": "Refero", "searchQuery": "site:refero.design {year}", "enabled": true },
  { "name": "Designspiration", "searchQuery": "site:designspiration.com web design {year}", "enabled": true }
]
```

- [ ] **Step 2: Create empty seen.json**
...
- [ ] **Step 3: Create initial taste-profile.md**
...
```bash
git add web/data/sources.json web/data/seen.json web/data/taste-profile.md
git commit -m "feat: add reference-search data files (sources, seen, taste-profile)"
```

---

### Task 2: 스킬 파일 — Phase 1 (소스 검색 + URL 수집)

**Files:**
- Create: `.claude/commands/reference-search.md`

- [ ] **Step 1: Create skill file with header + Phase 1 prompt**

`.claude/commands/reference-search.md` 생성. 전체 스킬 프롬프트의 시작 부분(헤더 + Phase 1)을 작성한다.
...
```bash
cat .claude/commands/reference-search.md | head -5
```
...
```bash
git add .claude/commands/reference-search.md
git commit -m "feat: add reference-search skill - Phase 1 (source search)"
```

---

### Task 3: 스킬 파일 — Phase 2 (분석 + 품질 게이트) 추가

**Files:**
- Modify: `.claude/commands/reference-search.md`

- [ ] **Step 1: Append Phase 2 to skill file**

`.claude/commands/reference-search.md` 끝에 Phase 2 프롬프트를 추가한다:
...
```bash
git add .claude/commands/reference-search.md
git commit -m "feat: add reference-search skill - Phase 2 (analysis + quality gates)"
```

---

### Task 4: 스킬 파일 — Phase 3 (큐레이션) 추가

**Files:**
- Modify: `.claude/commands/reference-search.md`

- [ ] **Step 1: Append Phase 3 to skill file**
...
```bash
git add .claude/commands/reference-search.md
git commit -m "feat: add reference-search skill - Phase 3 (curation)"
```

---

### Task 5: 스킬 파일 — Phase 4 (산출물 생성) 추가

**Files:**
- Modify: `.claude/commands/reference-search.md`

- [ ] **Step 1: Append Phase 4 to skill file**
...
```bash
git add .claude/commands/reference-search.md
git commit -m "feat: add reference-search skill - Phase 4 (catalog registration)"
```

---

### Task 6: 스킬 파일 — Phase 5 (회고 & 스킬 진화) 추가

**Files:**
- Modify: `.claude/commands/reference-search.md`

- [ ] **Step 1: Append Phase 5 to skill file**
...
```bash
git add .claude/commands/reference-search.md
git commit -m "feat: add reference-search skill - Phase 5 (retrospective & self-evolution)"
```
...
**Files:**
- Read: `.claude/commands/reference-search.md` (전체)
- Read: `web/data/sources.json`
- Read: `web/data/seen.json`
- Read: `web/data/taste-profile.md`

- [ ] **Step 1: 스킬 파일 전체 읽기**

Read로 `.claude/commands/reference-search.md` 전체를 읽고 아래 체크:
- Phase 1~5가 모두 존재하는가
- 라운드 루프 구조가 명확한가
- 각 Phase 간 데이터 흐름이 끊기지 않는가
- 모든 파일 경로가 정확한가 (web/data/sources.json 등)

- [ ] **Step 2: 데이터 파일 검증**

Read로 sources.json, seen.json, taste-profile.md를 읽고:
- sources.json이 유효한 JSON인가
- seen.json이 유효한 JSON이고 초기 상태인가
- taste-profile.md 초기 구조가 올바른가

- [ ] **Step 3: 스킬 실행 테스트 (dry run)**

Claude Code에서 `/reference-search`를 실행하여 Phase 1이 정상 시작하는지 확인.
WebSearch가 실제 결과를 반환하는지 1개 소스(Awwwards)로 테스트.
...
```bash
git add -A
git commit -m "feat: reference-search skill complete - 5-phase pipeline with self-evolution"
```
